...
看如下例子：
<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/9b332a96ea01401d901104ffb2e6d0c8.png#pic_center" width="100%" alt="window" /> </div>
...
<div style="text-align: center;">   <img src="https://img-blog.csdnimg.cn/direct/78fdf20e1e9c4beb80d8a24bb63349ff.png#pic_center)
...
<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/9413b48b3f15452cbb1906cfbb1b2198.png#pic_center" width="100%" alt="window" /> </div>


trace10:

<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/ca964939c3c3450bb30c983611d967bf.png#pic_center" width="100%" alt="window" /> </div>


trace11：

<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/381d11c1ffaa409e8f1267e346cafe58.png#pic_center" width="100%" alt="window" /> </div>

trace12:

<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/c9b039d8346e4a47a8983b63db024532.png#pic_center" width="100%" alt="window" /> </div>



trace13:

<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/e31a539ecb3f4d89add09ae37ee8bdaa.png#pic_center" width="100%" alt="window" /> </div>


trace14:

<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/59ff3a1e5c444b0d99c95b6be0a29911.png#pic_center" width="100%" alt="window" /> </div>

trace15:

<div style="text-align: center;">   <img src="一个基于C的shell实验.assets/077c5cf004c0411699f12307aa369d33.png#pic_center" width="100%" alt="window" /> </div>
...
int main() {
    ...
    deleteFirstNLines(FIindexAME, 7);
    char history[MAX_HISTORY][MAXARGS];
    int history_index = 0;
    int current_history = 0;
    int history_count = 0;
    char *delim;

    FILE *file = fopen(FIindexAME, "a+");
    if (file == NULL) {
    ...
    while (1) {
        ...
        clrtoeol();
        printw("Enter a string (Ctrl+L to clear, Enter to submit):\n$");
        refresh();
        ...
        while (1) {
            ...
            if (ch == '\n') {
            ...
            } else if (ch == 27) { // Escape key (arrow keys)
                char next1 = getch();
                char next2 = getch();
                if (next1 == '[' && next2 == 'A') { // Up arrow key
                ...
                } else if (next1 == '[' && next2 == 'B') { // Down arrow key
                    if (history_count > 0 && current_history < history_count - 1) {
                        current_history++;
                        strcpy(buffer, history[current_history]);
                        index = strlen(buffer);
                        move(getcury(stdscr), 0);
...
```c
void execveCommand(char *cmdline){
    int fd[2];
    pid_t pid;
    char *delim;
    int saved_fd;

    saved_fd = dup(STDIN_FILENO); // 保存标准输入

    delim = strchr(cmdline, '|');
    *delim = '\0'; // 将'|'替换为字符串结束符'\0'
...
void eval(char *cmdline) {
    char *argv[MAXARGS];
    char *redirection[3] = { NULL, NULL, NULL }; // Array to hold input and output redirection paths
    char buf[MAXARGS];
    int bg;
    pid_t pid;
    int fd_in, fd_out;
    ...
    strcpy(buf, cmdline);
    bg = parseline(buf, argv, redirection);

    if (argv[0] == NULL)
        return; /* Ignore empty line */

    // Check for redirection symbols
    char *input_redir = redirection[0];
    char *output_redir = redirection[1];
    ...
    if (!builtin_command(argv)) {
        if ((pid = Fork()) == 0) {
            if (input_redir || output_redir) {
                if (input_redir) {
                    fd_in = open(input_redir, O_RDONLY);
                    if (fd_in < 0) {
                ...
                if (output_redir) {
                    fd_out = open(output_redir, O_WRONLY | O_CREAT | O_TRUNC, 0644);
                    if (fd_out < 0) {
...
#include <errno.h>
#include <fcntl.h>
// gcc -o shell_pipe shell_pipe.c -l ncurses
#define MAXARGS 128
#define MAX_HISTORY 10
#define FIindexAME ".bash_history_"
extern char **environ;
...
void eval(char *cmdline);
int parseline(char *buf, char **argv, char **redirection);
...
int parseline(char *buf, char **argv, char **redirection) {
    char *delim;
    int argc;
    int bg = 0; // Initialize bg to 0 for foreground execution
    int param_flag = 2; // Initialize param_flag to indicate normal arguments
    ...
    while ((delim = strchr(buf, ' '))) {
        if (param_flag == 2) {
            ...
            *delim = '\0';
            ...
            redirection[param_flag] = buf;
            *delim = '\0';
            param_flag = 2; // Reset param_flag after handling redirection
        }
